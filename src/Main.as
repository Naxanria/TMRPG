    ...
    while (true)
    {
        ...
        if (map !is null && map.MapInfo.MapUid != "" && app.Editor is null)        
        {
            ...
            if (network.ClientManiaAppPlayground !is null)
            {
                ...
                personalBest = scoreMgr.Map_GetRecord_v2(userId, map.MapInfo.MapUid, "PersonalBest", "", "TimeAttack", "");
            }
            else
            {
                personalBest = -1;
...
void RenderMenu()
{
    string icon = (setting_show_pvm) ? Icons::Check : Icons::Times;
    if (UI::MenuItem("PVM " + icon))
    {
        setting_show_pvm = !setting_show_pvm;
    }
}
